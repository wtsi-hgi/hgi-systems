...
consul_datacenter="${region_setup}"
...
if [[ "${tstate}" == "ok" ]]; then
	...
	curl -o /dev/null --silent --fail --header "X-Consul-Token: ${TERRAFORM_CONSUL_TOKEN}" "http://consul.${region_setup}.hgi.sanger.ac.uk:8500/v1/kv/${key}?dc=${consul_datacenter}"
	echo "ok"
    done
