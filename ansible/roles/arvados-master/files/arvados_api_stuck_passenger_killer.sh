...
stuck_passenger_pids=$(passenger-status --show=xml | python <(echo -e "import xmltodict\nimport json\nimport sys\nprint(json.dumps(xmltodict.parse(sys.stdin.read())))")     | jq -r '.info.supergroups.supergroup.group.processes.process[] | select(.enabled == "DETACHED") | select(.uptime | split("m")[0] > 1) | .pid' | perl -p -e 's/[[:space:]]+/ /g')
