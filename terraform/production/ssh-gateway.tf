...
resource "openstack_compute_instance_v2" "ssh-gateway-hgi" {
  ...
  # provision using ansible
  provisioner "local-exec" {
    command = "ANSIBLE_CONFIG=../../ansible/ansible-minimal.cfg ansible-playbook -i ../../ansible/production_hosts.d -l 'openstack_compute_instance_v2.ssh-gateway-hgi' ../../ansible/site.yml"
  }
}
