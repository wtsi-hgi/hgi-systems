...
resource "openstack_compute_instance_v2" "arvados-shell" {
  provider    = "openstack"
  count       = 1
  name        = "arvados-shell-${var.arvados_cluster_id}"
  ...
  user_data = "#cloud-config\nhostname: arvados-shell-${var.arvados_cluster_id}\nfqdn: arvados-shell-${var.arvados_cluster_id}.${var.domain}"
...
resource "openstack_compute_floatingip_associate_v2" "arvados-shell" {
  provider    = "openstack"
  floating_ip = "${openstack_networking_floatingip_v2.arvados-shell.address}"
  instance_id = "${openstack_compute_instance_v2.arvados-shell.id}"
}

resource "infoblox_record" "arvados-shell" {
  value  = "${openstack_networking_floatingip_v2.arvados-shell.address}"
  name   = "arvados-shell-${var.arvados_cluster_id}"
  domain = "${var.domain}"
  type   = "A"
  ttl    = 600
