...
resource "openstack_compute_instance_v2" "arvados-sso" {
  ...
  metadata = {
    ansible_groups = "${join(" ", distinct(concat(["arvados-ssos", "arvados-cluster-${var.arvados_cluster_id}-members", "consul-agents", "hgi-credentials"], var.extra_ansible_groups)))}"
    user           = "${var.image["user"]}"
