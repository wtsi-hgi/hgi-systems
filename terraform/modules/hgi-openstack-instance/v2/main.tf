...
variable "auto_anti_affinity_name" {
  default = ""
}
...
locals {
  ...
  auto_anti_affinity_p = "${var.auto_anti_affinity_name != ""}"
...
resource "openstack_compute_instance_v2" "instance" {
  ...
  scheduler_hints {
    group = "${local.auto_anti_affinity_p ? (openstack_compute_servergroup_v2.auto-anti-affinity.*.id[0]) : ""}"
  }
...
resource "openstack_compute_servergroup_v2" "auto-anti-affinity" {
  count    = "${local.auto_anti_affinity_p ? 1 : 0}"
  name     = "${var.auto_anti_affinity_name}"
