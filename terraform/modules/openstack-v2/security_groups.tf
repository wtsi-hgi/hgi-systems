...
locals {
  security_groups = {
    ...
    consul-server  = "${openstack_networking_secgroup_v2.consul-server.id}"
    http           = "${openstack_networking_secgroup_v2.http.id}"
    http-cogs      = "${openstack_networking_secgroup_v2.http-cogs.id}"
    https          = "${openstack_networking_secgroup_v2.https.id}"
    ping           = "${openstack_networking_secgroup_v2.ping.id}"
    ssh            = "${openstack_networking_secgroup_v2.ssh.id}"
    postgres-local = "${openstack_networking_secgroup_v2.postgres-local.id}"
    tcp-local      = "${openstack_networking_secgroup_v2.tcp-local.id}"
    udp-local      = "${openstack_networking_secgroup_v2.udp-local.id}"
    slurm-master   = "${openstack_networking_secgroup_v2.slurm-master.id}"
    slurm-compute  = "${openstack_networking_secgroup_v2.slurm-compute.id}"
    keep-service   = "${openstack_networking_secgroup_v2.keep-service.id}"
