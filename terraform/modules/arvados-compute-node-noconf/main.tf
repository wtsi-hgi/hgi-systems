...
data "template_file" "docker-consul-script" {
  ...
  vars {
    ...
    CONSUL_ADVERTISE_ADDR = "${element(openstack_networking_port_v2.arvados-compute-port.*.all_fixed_ips[count.index], 0)}"
    ...
    CONSUL_BIND_ADDR      = "${element(openstack_networking_port_v2.arvados-compute-port.*.all_fixed_ips[count.index], 0)}"
  }
}
...
resource "openstack_compute_instance_v2" "arvados-compute" {
  ...
  provisioner "remote-exec" {
    ...
    connection {
      ...
      bastion_host = "${var.bastion["host"]}"
      bastion_user = "${var.bastion["user"]}"
      host         = "${element(openstack_networking_port_v2.arvados-compute-port.*.all_fixed_ips[count.index], 0)}"
    }
  }
}
